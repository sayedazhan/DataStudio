...
def _validate_forecast_upload(file: UploadFile) -> str:
    if not file.filename:
        raise HTTPException(status_code=400, detail="A forecast file with a filename is required.")
    extension = Path(file.filename).suffix.lower()
    if extension not in SUPPORTED_EXTENSIONS:
        raise HTTPException(status_code=400, detail="Forecast Studio supports CSV and XLSX files only.")
    return extension


@app.post("/api/datasets/forecast/prepare")
async def prepare_dataset_forecast(
    file: UploadFile = File(...),
    sheet_name: str | None = Form(default=None),
) -> dict[str, Any]:
    extension = _validate_forecast_upload(file)
    content = await _read_uploaded_bytes(file, FORECAST_MAX_FILE_SIZE)
    _validate_uploaded_format(content, extension)
    frame = _read_dataset(content, extension, sheet_name if extension == ".xlsx" else None)
    if frame.height == 0 or frame.width == 0:
        raise HTTPException(status_code=400, detail="The forecast dataset must contain rows and columns.")

    prepared = prepare_forecast(frame)
    prepared["dataset"] = {
        "filename": file.filename,
        "file_type": extension.removeprefix("."),
        "file_size_bytes": len(content),
...
@app.post("/api/datasets/forecast")
async def run_dataset_forecast(
    file: UploadFile = File(...),
    date_field: str = Form(...),
    metric_field: str = Form(...),
    aggregation: str = Form(default="sum"),
    frequency: str = Form(default="auto"),
